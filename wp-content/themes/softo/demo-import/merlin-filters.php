 ...
 */
function softo_unset_default_widgets_args( $widget_areas ) {
...
add_filter( 'merlin_unset_default_widgets_args', 'softo_unset_default_widgets_args' );
 ...
 */
function softo_child_functions_php( $output, $slug ) {
...
add_filter( 'merlin_generate_child_functions_php', 'softo_child_functions_php', 10, 2 );
 ...
 */
function softo_local_import_files() {
	return array(
		array(
			'import_file_name'         => esc_html__('Main Demo', 'softo'),
			...
			'local_import_redux'       => array(
				array(
					...
					'option_name' => 'softo_options',
				),
			),
			'local_import_file'        => trailingslashit( get_template_directory() ) . 'demo-import/content/content.xml',
			'import_preview_image_url' => get_template_directory_uri() . '/screenshot.png',
			'import_notice'            => esc_html__( 'After you import this demo, you will have to setup the slider separately.', 'softo' ),
			'preview_url'              => 'https://surielementor.com/softowp/',
		),
	);
}

add_filter( 'merlin_import_files', 'softo_local_import_files' );

/**
 * Execute custom code after the whole import has finished.
 */
function softo_after_import_setup() {
	// Assign menus to their locations.
	$main_menu = get_term_by( 'name', 'Main Menu', 'nav_menu' );
	$main_menu_2 = get_term_by( 'name', 'Home Three Menu', 'nav_menu' );

	set_theme_mod(
		'nav_menu_locations', array(
			'main_menu' => $main_menu->term_id,
			'main_menu_2' => $main_menu_2->term_id,
		)
	);
    ...
    if( class_exists('RevSliderSliderImport') ) {
		foreach(array('home-one', 'home-two', 'home-three') as $slider) {
...
add_action( 'merlin_after_all_import', 'softo_after_import_setup' );
