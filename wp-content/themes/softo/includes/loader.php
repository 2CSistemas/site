...
define( 'SOFTO_ROOT', get_template_directory() . '/' );
...
add_action( 'after_setup_theme', 'softo_wp_load', 5 );

function softo_wp_load() {

	defined( 'SOFTO_URL' ) or define( 'SOFTO_URL', get_template_directory_uri() . '/' );
	define(  'SOFTO_KEY','!@#softo');
	define(  'SOFTO_URI', get_template_directory_uri() . '/');

	if ( ! defined( 'SOFTO_NONCE' ) ) {
		define( 'SOFTO_NONCE', 'softo_wp_theme' );
	}

	( new \SOFTO\Includes\Classes\Base )->loadDefaults();
	( new \SOFTO\Includes\Classes\Ajax )->actions();

}
add_action( 'init', 'softo_bunch_theme_init');
function softo_bunch_theme_init()
{
