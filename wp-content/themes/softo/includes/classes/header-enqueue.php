...
class Header_Enqueue {
	...
	public static function scripts() {
		$options = get_theme_mod( 'softo' . '_options-mods' );
		$ssl     = is_ssl() ? 'https' : 'http';

		$scripts = array(
		);

		$scripts = apply_filters( 'SOFTO/includes/classes/header_enqueue/scripts', $scripts );
		/**
		 * Enqueue the scripts
		 *
		 * @var array
		...
		$header_data = array(
			...
			'nonce'   => wp_create_nonce( SOFTO_NONCE ),
		);

		wp_localize_script( 'jquery', 'softo_data', $header_data );

		if ( softo_set( $options, 'footer_js' ) ) {

			wp_add_inline_script( 'jquery', softo_set( $options, 'footer_js' ) );
		}
	...
	public static function styles() {
		...
		$styles = apply_filters( 'SOFTO/includes/classes/header_enqueue/styles', $styles );
		...
		foreach ( $styles as $name => $style ) {
			...
			if ( strstr( $style, 'http' ) || strstr( $style, 'https' ) || strstr( $style, 'fonts.googleapis' ) ) {
				wp_enqueue_style( "softo-{$name}", $style );
			} else {
				wp_enqueue_style( "softo-{$name}", get_template_directory_uri() . '/' . $style );
			}
		}
		$options      = softo_WSH()->option();
		$custom_style = '';
		...
		if ( $custom_font = $options->get('theme_custom_font') ) {
            $header_styles .= softo_custom_fonts_load( $custom_font );
        }

		wp_add_inline_style( 'softo-main-style', $header_styles );
	}
	...
	public static function resource_hints( $urls, $relation_type ) {
		if ( wp_style_is( 'softo-fonts', 'queue' ) && 'preconnect' === $relation_type ) {
			$urls[] = array(
	...
	public static function header_styles() {
		...
		$data = \SOFTO\Includes\Classes\Common::instance()->data( 'blog' )->get();
		...
		$settings = get_theme_mod( 'softo' . '_options-mods' );

		if ( $custom_font = softo_set( $settings, 'theme_custom_font' ) ) {

			$styles .= apply_filters('softo_redux_custom_fonts_load', $custom_font );
