...
class Common {
	...
	function blog( $template = 'blog' ) {
		...
		if ( ( $wp_query->is_posts_page && 'blog' == $template ) || $template == 'page' ) {
			...
			$return = [
				'layout'            => softo_meta( 'sidebar_sidebar_layout', $page_id, 'right' ),
				'sidebar_type'      => softo_meta( 'sidebar_source_type', $page_id, 'd' ),
				'sidebar_elementor' => softo_meta( 'sidebar_elementor_template', $page_id ),
				'sidebar'           => softo_meta( 'sidebar_page_sidebar', $page_id, 'default-sidebar' ),
				'banner_type'       => softo_meta( 'banner_source_type', $page_id ),
				...
				'title'             => softo_meta( 'banner_banner_title', $page_id ),
				'text'             => softo_meta( 'banner_banner_text', $page_id ),
			...
			$return = [
				...
				'title'         => $options->get( $title ) ? $options->get( $title ) : softo_the_title( $template ),
				'text'         => $options->get( $text ) ? $options->get( $text ) : softo_the_title( $template ),
				'banner'        => softo_set( $bg, 'url' ),
