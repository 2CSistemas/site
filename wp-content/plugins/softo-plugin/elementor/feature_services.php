...
class Feature_Services extends Widget_Base {
	...
	public function get_title() {
		return esc_html__( 'Feature Services', 'softo' );
	}
	...
	protected function _register_controls() {
		...
		$this->add_control(
			...
			[
				...
				'default' => 'one',
				'options' => array(
					'one' => esc_html__( 'Choose Without Overlap Section', 'softo' ),
					'two' => esc_html__( 'Choose With Overlap Section ', 'softo' ),
				),
			]
		);
	...
	<section class="features <?php if($settings['style_two'] == 'two') echo 'features-style1'; else echo 'flat-row'; ?>">
        <div class="container d-lg-flex">
			<?php 
				$count = 1;
            	while ( $query->have_posts() ) : $query->the_post(); 
            ?>
            <div class="iconbox-features col-lg-4 hv-background-before <?php if($count == 2) echo 'active has-scale'; ?>">
                <div class="iconbox-icon"><span class="<?php echo wp_kses(str_replace( "icon ",  "", get_post_meta(get_the_id(), 'service_icon', true )), true); ?>"></span></div>
                <div class="iconbox-content">
                    ...
                    <div class="discover-more">
                        <a href="<?php echo esc_url(get_post_meta( get_the_id(), 'service_url', true ));?>"><?php esc_html_e('Discover More', 'softo'); ?></a>
                    </div>
                </div>
            </div>
			<?php $count++; endwhile; ?>
